...
class DavisAugData(Dataset):
    def __init__(self, data_root, mask, in_channs=1):
        if not osp.isdir(data_root):
            raise FileNotFoundError(f"Training data directory does not exist: {data_root}")
    ...
    def __getitem__(self,index):
        ...
        # begin_h = np.random.randint(0,image_h-self.resize_h)
        # begin_w = np.random.randint(0,image_w-self.resize_w)
        # end_h = begin_h+ self.resize_h
        # end_w = begin_w+ self.resize_w
